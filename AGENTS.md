...
- `RanPAC` cache version is `15` in `classifiers/ranpac.py`.
- `HiRA` cache version is `30` in `classifiers/hira.py`.
...
- Existing RanPAC caches before version `15` and HiRA caches before version `30` are stale for the current GELU head with pre-projection soft-threshold statistics, and should be retrained.

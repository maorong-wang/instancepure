...
Shared working idea: robustness gains may come from injecting large random projections while preserving the base backbone. Treat RanPAC as the reference head. For HiRA experiments, prefer ViT-family pre-MLP attachment on the last `hira_num_blocks` transformer blocks (default `2`): insert a feature adapter before each target MLP, using `B(GELU(A(x)))` during fitting and `B(sparse(GELU(A(x))))` during inference, with fixed Gaussian `A` and closed-form ridge fit of `B` against the frozen original MLP inputs. Do not switch HiRA `A` or RanPAC `W_rand` to block-orthogonal initialization unless explicitly revisiting that ablation.
...
- HiRA now attaches before each target MLP, reconstructs the original MLP input during closed-form fitting, and applies MeanSparse only at inference on the hidden `GELU(A(x))` activations.
...
Our main method is to modify the victim classifier rather than the purifier. `RanPAC` replaces the final linear layer with a random-projection ridge head from [classifiers/ranpac.py](/home_fmg/maorong/python/InstantPure/classifiers/ranpac.py): backbone features are projected by fixed Gaussian `W_rand`, passed through `GELU`, and fit by ridge regression. The cached state now includes the RP weights, the chosen ridge, the hard-negative target settings, the noise-adaptation settings, the stability-aware diagonal ridge settings, and a scalar clean-train baseline-logit mean used at inference time. `HiRA` lives in [classifiers/hira.py](/home_fmg/maorong/python/InstantPure/classifiers/hira.py): for ViT-family backbones only, keep the original MLP and attach a pre-MLP adapter in the last `hira_num_blocks` transformer blocks. The adapter uses fixed Gaussian `A`, computes `GELU(A(x))`, applies inference-only MeanSparse thresholding in that hidden space, then maps back with ridge-fit `B` before the original MLP. `B` is solved by closed-form ridge regression to match the original frozen MLP inputs, with optional stability-aware diagonal ridge regularization from the same train-pass projected statistics. Current preference is Gaussian initialization; do not use block-orthogonal init.

Purification-based evaluation is in [test.py](/home_fmg/maorong/python/InstantPure/test.py). It loads a victim classifier through [archs.py](/home_fmg/maorong/python/InstantPure/archs.py), optionally applies HiRA and/or RanPAC, samples a random ImageNet `test` subset, attacks the classifier directly, and reports both baseline and purified metrics. Key outputs are raw classifier clean accuracy (`classifier_accuracy`), raw classifier robust accuracy under the attack (`original_classifier_robust_accuracy`), purified clean accuracy, and purified robust accuracy after the diffusion denoiser. Attacks include Foolbox PGD, AutoAttack, and `stadv` imported from `../DiffPure`. `advertorch` is not used. The InstantPure CLI currently exposes the RanPAC mixing / hard-negative knobs, the HiRA-only inference soft-threshold knobs, and the stability-aware ridge knobs, and RanPAC variants are tagged with `_bbias` in output paths.
...
- Existing RanPAC caches before version `17` and HiRA caches before version `32` are stale for the current no-threshold RanPAC head and pre-MLP HiRA hidden-threshold design with stability-aware diagonal ridge support, and should be retrained.

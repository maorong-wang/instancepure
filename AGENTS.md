...
- `RanPAC` cache version is `16` in `classifiers/ranpac.py`.
...
- Existing RanPAC caches before version `16` and HiRA caches before version `31` are stale for the current GELU head with pre-projection soft-threshold statistics plus stability-aware diagonal ridge support, and should be retrained.
